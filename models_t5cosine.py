...
class T5CosineForNTR(T5ForConditionalGeneration):
    ...
    def forward(
        ...
        if labels is not None:
            ...
            weight = kl_weight(self.config.annealing_fn, steps, self.config.k, self.config.x0)

            loss_cosine_w = weight * loss_cosine
            loss = loss_ce + loss_cosine_w

            if steps % 20 == 0:
                print(f"NLL: {loss_ce}")
                print(f"COSINE: {loss_cosine_w} = {loss_cosine} * {weight}")
                with torch.no_grad():
